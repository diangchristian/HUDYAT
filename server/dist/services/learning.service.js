...
export const getLearningAreas = async (learnerId) => {
    ...
    return learningAreas.map((area) => ({
        ...
        categories: area.categories.map((category) => {
            ...
            if (rawStatus === "COMPLETED") {
                ...
                totalGestures > 0) {
                progressPercent = Math.round((((progress?.lastGestureIndex ?? 0) + 1) /
                    totalGestures) *
                    100);
            }
            /*
            ...
            return {
                ...
                progress: {
                    status: rawStatus,
                    lastGestureIndex: progress?.lastGestureIndex ?? 0,
                    lastLessonStep: progress?.lastLessonStep ?? null,
...
export const getCategoryLesson = async (categoryId, learnerId) => {
    const [category, categoryGestures, progress,] = await Promise.all([
...
export const saveLessonCheckpoint = async (categoryId, learnerId, gestureIndex, lessonStep) => {
    ...
    if (category.gestures.length === 0) {
        throw new Error("This category has no lessons yet.");
    }
    if (gestureIndex < 0 ||
     ...
     */
    const isFinalStep = gestureIndex === category.gestures.length - 1 &&
        lessonStep === "try";
