 ...
 */
const assertLessonIsUnlocked = async (categoryId, learnerId) => {
    ...
    const isUnlocked = categoryProgress?.lessonCompletedAt != null ||
        categoryProgress?.status === "COMPLETED";
    if (!isUnlocked) {
        const error = new Error("Finish this category's lesson before taking its assessment.");
 ...
 */
export const checkAnswer = async (categoryId, learnerId, questionId, selectedChoiceId) => {
    ...
    const correctChoice = question.choices.find((choice) => choice.gestureId === question.gestureId);
    return {
        isCorrect: selectedChoice.gestureId === question.gestureId,
        correctChoiceId: correctChoice?.id ?? null,
    };
};
export const getAssessmentByCategory = async (categoryId, learnerId) => {
    const assessment = await prisma.assessment.findUnique({
        where: {
            categoryId,
        },
...
export const submitAssessment = async (categoryId, learnerId, answers) => {
    await assertLessonIsUnlocked(categoryId, learnerId);
    return prisma.$transaction(async (tx) => {
        ...
        if (uniqueQuestionIds.size !== answers.length) {
            throw new Error("Duplicate answers were submitted.");
        }
        const questionMap = new Map(assessment.questions.map((question) => [
            question.id,
            question,
        ]));
        ...
        const evaluatedAnswers = answers.map((answer) => {
            ...
            const selectedChoice = question.choices.find((choice) => choice.id ===
                answer.selectedChoiceId);
            if (!selectedChoice) {
                throw new Error("Invalid answer choice.");
            }
            const isCorrect = selectedChoice.gestureId ===
                question.gestureId;
            const pointsEarned = isCorrect
                ? new Prisma.Decimal(question.points)
                : new Prisma.Decimal(0);
            totalPoints =
                totalPoints.add(question.points);
            if (isCorrect) {
                score = score.add(question.points);
            }
            return {
        ...
        const percentage = totalPoints.gt(0)
            ? score
                .div(totalPoints)
                .mul(100)
            : new Prisma.Decimal(0);
        const passed = percentage.gte(assessment.passingScore);
        ...
        if (passed) {
            ...
            const nextCategory = await tx.category.findFirst({
                where: {
                    learningAreaId: assessment.category
                        .learningAreaId,
                    isActive: true,
                    displayOrder: {
...
export const getAssessmentsForLearner = async (learnerId) => {
    const [learningAreas, progressRecords, completedAttempts] = await Promise.all([
    ...
    const progressByCategoryId = new Map(progressRecords.map((progress) => [
        progress.categoryId,
        progress,
    ]));
    ...
    for (const attempt of completedAttempts) {
        ...
        const existing = attemptsByCategoryId.get(categoryId) ?? [];
    ...
    return {
        learningAreas: learningAreas
            ...
            categories: area.categories
                .filter((category) => category.assessment !== null)
                .map((category) => {
                const assessment = category.assessment;
                const progress = progressByCategoryId.get(category.id);
                const isUnlocked = progress?.lessonCompletedAt != null ||
                    progress?.status === "COMPLETED";
                const attempts = attemptsByCategoryId.get(category.id) ?? [];
                const attemptPassed = (attempt) => attempt.totalPoints.gt(0) &&
                    attempt.score
