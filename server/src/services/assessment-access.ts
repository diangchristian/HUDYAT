...
export function assessmentAccess(items:readonly {passed:boolean;published:boolean;questionCount:number}[]){
 ...
 return items.map(item=>{
  ...
  return {isUnlocked:unlocked,lockedReason,status:(item.passed?'completed':unlocked?'not-started':'locked') as 'completed'|'not-started'|'locked'};
 });
}
