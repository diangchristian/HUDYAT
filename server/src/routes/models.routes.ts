...
router.get('/:category',async(req,res)=>{
 try {
  ...
  if(!category || !category.modelRelease)return res.status(404).json({message:'No model has been published for this category yet.'});
  res.setHeader('Cache-Control','no-store');
  return res.json({data:category.modelRelease});
 } catch {return res.status(503).json({message:'Model catalog is temporarily unavailable.'});}
});
