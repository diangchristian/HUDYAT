...
export function CategoryRecognition({category,targetLabel,video,active,onCorrect,onWrong,onStatus,retry}:{category:string;targetLabel?:string;video:RefObject<HTMLVideoElement|null>;active:boolean;onCorrect:()=>void;onWrong:()=>void;onStatus:(status:RecognitionStatus)=>void;retry:number}){
 ...
 useEffect(()=>{
  ...
  const run=async()=>{
   ...
   try{
    if(document.hidden){lock=emptyLock();previous=undefined;frames=[];start=0;}
     ...
     if(current.category==='alphabet'){
      ...
      if(cancelled)return;
      lock=advanceLock(lock,{now:performance.now(),present,label:result?.prediction,score:result?.score,margin:result?.margin,steady:isSteady(previous,frame),maxGapMs:Math.max(600,previousProcessingMs+(performance.now()-frameStarted)+350)});
   ...
   }catch(error){if(!cancelled){lock=emptyLock();frames=[];start=0;notifyStatus({phase:'error',message:error instanceof Error?error.message:'Recognition failed.'});}}
 ...
 },[active,ready,category,targetLabel,video,retry]);
 return null;
}
